...
(function () {
    ...
    for(let i=0; i<elems.length; i++) {
      ...
      elems[i].innerHTML = genHTML(genSanskrit(txt), "ITRANS");
    }
})();
...
function genHTML(txt, btnTxt) {
  return `<span class="source-switch" onclick="swapITRANS(this.parentNode)">${btnTxt}</span>${txt}`;
}

function swapITRANS(e) {
  let chld = e.getElementsByTagName("span")[0];
  let btnTxt = chld.innerText == "ITRANS" ? "Sanskrit" : "ITRANS"
  e.removeChild(chld)
  var old = e.innerText;
